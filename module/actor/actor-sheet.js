...
export class HeroSystem6eActorSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      tabs: [
        ...
        {
          navSelector: '.sheet-edit-tabs',
          contentSelector: '.sheet-mode',
          initial: 'play'
        }
      ]
    })
  ...
  async _onRecovery(event) {
    let newStun =
      this.actor.data.data.stun.value + this.actor.data.data.characteristics['rec'].current
    let newEnd =
      this.actor.data.data.end.value + this.actor.data.data.characteristics['rec'].current
  ...
  async _applyCharacterSheetAsync(sheet) {
    ...
    for (let characteristic of characteristics.children) {
      ...
      changes[`data.characteristics.${key}.value`] =
        CONFIG.HERO.characteristicDefaults[key] + parseInt(characteristic.getAttribute('LEVELS'))
    }
    ...
    for (let skill of skills.children) {
      ...
      if (skill.attributes.getNamedItem('CHARACTERISTIC')) {
        data['characteristic'] =
          CONFIG.HERO.characteristicsXMLKey[skill.getAttribute('CHARACTERISTIC')]
      } else {
        data['characteristic'] = ''
      }
